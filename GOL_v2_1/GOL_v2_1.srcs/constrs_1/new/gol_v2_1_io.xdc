set_property PACKAGE_PIN A3 [get_ports {pixel[11]}]
set_property PACKAGE_PIN B4 [get_ports {pixel[10]}]
set_property PACKAGE_PIN C5 [get_ports {pixel[9]}]
set_property PACKAGE_PIN A4 [get_ports {pixel[8]}]
set_property PACKAGE_PIN C6 [get_ports {pixel[7]}]
set_property PACKAGE_PIN A5 [get_ports {pixel[6]}]
set_property PACKAGE_PIN B6 [get_ports {pixel[5]}]
set_property PACKAGE_PIN A6 [get_ports {pixel[4]}]
set_property PACKAGE_PIN B7 [get_ports {pixel[3]}]
set_property PACKAGE_PIN C7 [get_ports {pixel[2]}]
set_property PACKAGE_PIN D7 [get_ports {pixel[1]}]
set_property PACKAGE_PIN D8 [get_ports {pixel[0]}]
set_property PACKAGE_PIN E3 [get_ports clk]
set_property PACKAGE_PIN B11 [get_ports hsync]
set_property PACKAGE_PIN B12 [get_ports vsync]
set_property PACKAGE_PIN N17 [get_ports rst]

set_property IOSTANDARD LVCMOS33 [get_ports vsync]
set_property IOSTANDARD LVCMOS33 [get_ports rst]
set_property IOSTANDARD LVCMOS33 [get_ports hsync]
set_property IOSTANDARD LVCMOS33 [get_ports clk]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[11]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[10]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[9]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[8]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[7]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[6]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[5]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[4]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[3]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[2]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[1]}]
set_property IOSTANDARD LVCMOS33 [get_ports {pixel[0]}]

set_property PACKAGE_PIN J15 [get_ports step_mode]
set_property PACKAGE_PIN P18 [get_ports step_button]
set_property IOSTANDARD LVCMOS33 [get_ports step_button]
set_property IOSTANDARD LVCMOS33 [get_ports step_mode]


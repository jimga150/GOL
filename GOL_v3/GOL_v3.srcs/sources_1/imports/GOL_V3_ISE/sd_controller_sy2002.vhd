...
entity sd_controller_sy2002 is
generic (
...
port (
	...
	card_present : in std_logic := '1';	-- From socket - can be fixed to '1' if no switch is present
	card_write_prot : in std_logic := '1';	-- From socket - can be fixed to '0' if no switch is present, or '1' to make a Read-Only interface

	rd : in std_logic := '0';				-- Trigger single block read
	rd_multiple : in std_logic := '0';		-- Trigger multiple block read
	dout : out std_logic_vector(7 downto 0);	-- Data from SD card
	dout_avail : out std_logic;		-- Set when dout is valid
	dout_taken : in std_logic;		-- Acknowledgement for dout
	
	wr : in std_logic := '0';				-- Trigger single block write
	wr_multiple : in std_logic := '0';		-- Trigger multiple block write
	din : in std_logic_vector(7 downto 0) := (others => '0');	-- Data to SD card
	din_valid : in std_logic := '0';		-- Set when din is valid
	din_taken : out std_logic;		-- Ackowledgement for din
	
	addr : in std_logic_vector(31 downto 0);	-- Block address
	erase_count : in std_logic_vector(7 downto 0) := (others => '0'); -- For wr_multiple only
...
end sd_controller_sy2002;

architecture rtl of sd_controller_sy2002 is
type states is (
